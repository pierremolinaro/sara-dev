...
pr_explicit_automaton_sara_parser_1495_26_ (sara_scanner & lexique_var_,
  ...
  GGS_M_stateMap::element_type * operand_48237 = var_cas_statesMap.firstObject () ;
  while ((operand_48237 != NULL)) {
    macroValidPointer (operand_48237) ;
    if (((! var_cas_stateDefinitionMap.reader_hasKey (operand_48237->mKey.reader_value ()))).isBuiltAndTrue ()) {
        GGS_location (lexique_var_).reader_location ().signalGGSSemanticError (lexique_var_, ((((GGS_string (true, "'")) + (operand_48237->mKey.reader_value ()))) + (GGS_string (true, "' state is not described")))) ;
    }
    operand_48237 = operand_48237->nextObject () ;
  }
  lexique_var_.acceptTerminal (sara_scanner::sara_scanner_1__7D) ;
