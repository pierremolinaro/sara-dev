...
pr_explicit_automaton_sara_parser_1073_6_ (sara_scanner & _inLexique,
  ...
  GGS_M_stateMap::element_type * operand_35707 = var_cas_statesMap.firstObject () ;
  while ((operand_35707 != NULL)) {
    macroValidPointer (operand_35707) ;
    if (((var_cas_stateDefinitionMap.reader_hasKey (_inLexique, operand_35707->mKey.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (1158)) COMMA_SOURCE_FILE_AT_LINE (1158)))._operator_not ()).isBuiltAndTrue ()) {
      GGS_location (_inLexique).reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, ((GGS_string (true, "'"))._operator_concat (operand_35707->mKey.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (1159))))._operator_concat (GGS_string (true, "' state is not described")) COMMA_SOURCE_FILE_AT_LINE (1160)) ;
    }
    operand_35707 = operand_35707->nextObject () ;
  }
  _inLexique.acceptTerminal (ACCEPT_TERMINAL (sara_scanner::sara_scanner_1__7D) COMMA_HERE) ;
