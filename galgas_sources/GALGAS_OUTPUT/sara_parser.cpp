...
pr_explicit_automaton_sara_parser_531_26_ (sara_scanner & lexique_var_,
  ...
  GGS_M_stateMap::element_type * operand_18938 = var_cas_statesMap.getFirstItem () ;
  while (operand_18938 != NULL) {
    macroValidPointer (operand_18938) ;
    if (((! var_cas_stateDefinitionMap.reader_hasKey (operand_18938->mKey))).isBuiltAndTrue ()) {
        GGS_location (lexique_var_).reader_location ().signalGGSSemanticError (lexique_var_, ((((GGS_string (true, "'")) + (operand_18938->mKey.reader_value ()))) + (GGS_string (true, "' state is not described")))) ;
    }
    operand_18938 = operand_18938->getNextItem () ;
  }
  lexique_var_.acceptTerminal (sara_scanner::sara_scanner_1__7D) ;
