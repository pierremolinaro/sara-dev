...
//---------------------------------------------------------------------------*
  ...
  #define SOURCE_FILE_AT_LINE(line) "sara_main.ggs", line
...
int mainForLIBPM  (const int argc, const char * argv []) {
  ...
  //--- Input/output parameters
    ...
    const char * extensions [] = {"sara", NULL} ;
    const char * helpMessages [] = {"an '.sara' source file", NULL} ;
  ...
  //--- Enable 64 bit alloc debug ? Only if compiled in 64 bit and in debug mode
    ...
    try{
      verboseOptionOn = options.boolOptionValueFromKeys ("generic_galgas_cli_options",
                                                         "verbose_output",
                                                         false) ;
      sara_main_prologue (options, sourceFilesArray) ;
      for (sint32 i=0 ; i<sourceFilesArray.count () ; i++) {
        const C_String fileExtension = sourceFilesArray (i COMMA_HERE).pathExtension () ;
        sint16 r = 0 ;
        if (fileExtension.compare ("sara") == 0) {
          ...
          { sara_scanner * scanner_ = NULL ;
            ...
            if (sourceFileName.fileExists ()) {
              macroMyNew (scanner_, sara_scanner (_inLexique.ioParametersPtr (), sourceFileName COMMA_HERE)) ;
              ...
              try{
                ...
                var_cas_inSourceFile.signalSemanticError (_inLexique, inFileReadError.what () COMMA_SOURCE_FILE_AT_LINE (13)) ;
              }
              macroDetachPointer (scanner_, sara_scanner) ;
            }else{
              C_String message ;
              message << "The '" << sourceFileName << "' file does not exist" ;
              var_cas_inSourceFile.signalSemanticError (_inLexique, message.cString () COMMA_SOURCE_FILE_AT_LINE (13)) ;
            }
          }
