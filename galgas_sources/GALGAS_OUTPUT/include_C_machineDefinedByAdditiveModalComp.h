//---------------------------------------------------------------------------*
//                                                                           *
//           File 'include_C_machineDefinedByAdditiveModalComp.h'            *
//         Generated by version GALGAS_BETA_VERSION (LL(1) grammar)          *
//                      march 12th, 2006, at 21h54'26"                       *
//                                                                           *
//---------------------------------------------------------------------------*

//--- START OF USER ZONE 1


//--- END OF USER ZONE 1

//---------------------------------------------------------------------------*
//                                                                           *
//             class 'cPtr_C_machineDefinedByAdditiveModalComp'              *
//                                                                           *
//---------------------------------------------------------------------------*

#ifndef C_machineDefinedByAdditiveModalComp_DEFINED
#define C_machineDefinedByAdditiveModalComp_DEFINED

class cPtr_C_machineDefinedByAdditiveModalComp : public cPtr_AC_job {
  private : typedef cPtr_AC_job inherited ;
//--- START OF USER ZONE 2

  public : virtual void compute (C_Lexique & inLexique,
                                 TC_Array <C_saraMachine> & ioSaraSystemArray,
                                 const bool inDisplayBDDvaluesCount,
                                 const bool inDisplayBDDvalues) const ;

//--- END OF USER ZONE 2

  public : cPtr_C_machineDefinedByAdditiveModalComp (const GGS_lstring & ,
                                const GGS_luint & ,
                                const GGS_uint & ,
                                const GGS_uint & ,
                                const GGS_M_variablesMap & ,
                                const GGS_M_modesMap & ,
                                const GGS_ListForModes &  COMMA_LOCATION_ARGS) ;
  public : GGS_lstring  mMachineName ;
  public : GGS_luint  mMachineIndex ;
  public : GGS_uint  mInputVariableCount ;
  public : GGS_uint  mInputAndInternalVariableCount ;
  public : GGS_M_variablesMap  mVariablesMap ;
  public : GGS_M_modesMap  mModeMap ;
  public : GGS_ListForModes  mInclusionList ;
} ;

//---------------------------------------------------------------------------*

class GGG_C_machineDefinedByAdditiveModalComp : public GGG_AC_job {
  private : typedef GGG_AC_job inherited ;
  public : GGG_C_machineDefinedByAdditiveModalComp (const GGS_lstring & ,
                                const GGS_luint & ,
                                const GGS_uint & ,
                                const GGS_uint & ,
                                const GGS_M_variablesMap & ,
                                const GGS_M_modesMap & ,
                                const GGS_ListForModes &  COMMA_LOCATION_ARGS) ;
} ;

//---------------------------------------------------------------------------*

#endif

