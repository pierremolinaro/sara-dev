...
cPtr_C_modalComposition::cPtr_C_modalComposition (const GGS_AC_machineDefinition & argument_0,
                                const GGS_location & argument_1,
                                const GGS_AC_machineDefinition & argument_2)
:mLeftOperand (argument_0),
...
bool cPtr_C_modalComposition::isBuilt (void) const {
  return mLeftOperand.isBuilt () &&
...
#ifndef DO_NOT_GENERATE_MEMORY_CHECK_CODE
  cPtr_C_modalComposition * GGS_C_modalComposition::
    constructor_new (const GGS_AC_machineDefinition & argument_0,
                                const GGS_location & argument_1,
                                const GGS_AC_machineDefinition & argument_2) {
    cPtr_C_modalComposition * ptr_ = (cPtr_C_modalComposition *) NULL ;
    macroMyNew (ptr_, cPtr_C_modalComposition (argument_0,
                                argument_1,
                                argument_2)) ;
    return ptr_ ;
  }
...
insertElement (C_lexique & inLexique,
  ...
  if (isBuilt () && inKey.isBuilt ()
    ...
    info.mVariablesMap = inParameter1 ;
    info.mDefinition = inParameter2 ;
    bool extension = false ; // Unused here
    internalInsert ((void * ) & inKey, (void *) & info, mRoot, extension, index) ;
    if (index < 0) {
      C_string keyString ;
...
searchElement (C_lexique & inLexique,
  ...
  if (node == NULL) {
    ...
    if (outParameter1 != NULL) {
      * outParameter1 = node->mInfo.mVariablesMap ;
    }
    if (outParameter2 != NULL) {
      * outParameter2 = node->mInfo.mDefinition ;
    }
    if (outIndex != NULL) {
      * outIndex = GGS_luint (GGS_uint (true, (uint32) node->mIndex), inKey) ;
    }
