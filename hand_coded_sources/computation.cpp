...
computeBDD (C_Compiler & /* inLexique */,
  ...
  return C_BDD ((PMUInt16) (mInputVarIndex.uintValue () + inBDDslotOffset), true) ;
}
...
//--- Get index of imported machine
  const PMSInt32 indexOfImportedMachine = (PMSInt32) mIndexOfImportedMachine.uintValue () ;
...
//--- Construct substitution arraies
  ...
  while (p != NULL) {
    macroValidPointer (p) ;
    statesSubstitutionArray [index] = (PMUInt16) p->mTargetSlot.uintValue () ;
    p = p->nextObject () ;
    index ++ ;
  }
...
//--- Build input variables array names
  const PMUInt16 inputVariablesCount = (PMUInt16) mInputVariableCount.uintValue () ;
  machine.mInputVariablesCount = inputVariablesCount ;
  const PMUInt16 variablesCount = (PMUInt16) mVariablesMap.count () ;
  const PMUInt16 inputAndInternalVariablesCount = (PMUInt16) mInputAndInternalVariableCount.uintValue () ;
...
compute (C_Compiler & /* inLexique */,
         ...
         const bool /* inDisplayBDDvalues */) const {
//--- Get machine index
  const PMSInt32 machineIndex1 = (PMSInt32) mMachineIndex1.uintValue () ;
  const PMSInt32 machineIndex2 = (PMSInt32) mMachineIndex2.uintValue () ;
...
compute (C_Compiler & /* inLexique */,
         ...
         const bool /* inDisplayBDDvalues */) const {
 co << "------------------ Scenarios for '"
    << ioSaraSystemArray ( (PMSInt32) mMachineIndex.uintValue () COMMA_HERE).mMachineName
    << "' machine\n" ;
//--- Initial state BDD
  const C_BDD initialStateBDD = ioSaraSystemArray ((PMSInt32) mMachineIndex.uintValue () COMMA_HERE).mInitialStatesBDD ;
//--- transitions BDD
  const C_BDD transitionsBDD = ioSaraSystemArray ((PMSInt32) mMachineIndex.uintValue () COMMA_HERE).mTransitionRelationBDD ;
//--- variables count
  const PMUInt16 variableCount = (PMUInt16) ioSaraSystemArray ((PMSInt32) mMachineIndex.uintValue () COMMA_HERE).mNamesArray.count () ;
//--- Loop throuhgt all scenarios
  ...
  //--- Build initial configuration
    ...
    while (v != NULL) {
      macroValidPointer (v) ;
      initialConfiguration += ((PMUInt64) v->mInputValue.uintValue ()) << shift ;
      shift ++ ;
      v = v->nextObject () ;
    }
  ...
  //--- Display initial configuration
    currentState.printBDD (ioSaraSystemArray ((PMSInt32) mMachineIndex.uintValue () COMMA_HERE).mNamesArray, 3) ;
  ...
  //--- Loop throught input sequence
    ...
    //--- Parse new input configuration
      ...
      while (v != NULL) {
        macroValidPointer (v) ;
        inputConfiguration += ((PMUInt64) v->mInputValue.uintValue ()) << shift ;
        shift ++ ;
        v = v->nextObject () ;
      }
...
//----------------------------------------------------------------------- Check that states are disjoined
  ...
  while (currentDefinition != NULL) {
    ...
    while (testedState != NULL) {
      ...
      if (! (stateExpressionBDD ((PMSInt32) currentDefinition->mStateIndex.uintValue () COMMA_HERE)
           & stateExpressionBDD ((PMSInt32) testedState->mStateIndex.uintValue () COMMA_HERE)).isFalse ()) {
        C_String errorMessage ;
        errorMessage << "expression for state '"
                   << stateNameArray ((PMSInt32) testedState->mStateIndex.uintValue () COMMA_HERE)
...
//--- Compute BDD initial states
  ...
  while (currentInitialState != NULL) {
    ...
    outInitialStatesBDD |= stateExpressionBDD ((PMSInt32) currentInitialState->mStateIndex.uintValue () COMMA_HERE) ;
...
//--- Check initial states are disjoined
  ...
  while (currentInitialState != NULL) {
    ...
    while (testedInitialState != NULL) {
      ...
      const C_BDD intersection = stateExpressionBDD ((PMSInt32) currentInitialState->mStateIndex.uintValue () COMMA_HERE)
        & stateExpressionBDD ((PMSInt32) testedInitialState->mStateIndex.uintValue () COMMA_HERE) ;
      if (! intersection.isFalse ()) {
        C_String errorMessage ;
        errorMessage << "initial state '"
                     << stateNameArray ((PMSInt32) testedInitialState->mStateIndex.uintValue () COMMA_HERE)
...
//--- Compute BDD initial states
  ...
  while (currentTerminalState != NULL) {
    ...
    outTerminalStatesBDD |= stateExpressionBDD ((PMSInt32) currentTerminalState->mStateIndex.uintValue () COMMA_HERE) ;
...
//     state input configuration and state output configuration
  ...
  //--- Get current state index
    const PMSInt32 currentStateIndex = (PMSInt32) currentDefinition->mStateIndex.uintValue () ;
...
//--- Check transitions of each state
  ...
  //--- Check that action does not intersect with state input expression
    ...
    //--- Check that action is compatible input configuration of target state
      const C_BDD x = actionBDD & stateExpressionBDD ((PMSInt32) currentTransition->mTargetStateIndex.uintValue () COMMA_HERE) ;
      if (x.isFalse ()) {
        C_String errorMessage ;
...
computeFromExpression (C_Compiler & inLexique,
                       ...
                       C_BDD & outTerminalStatesBDD,
                       C_BDD & outAccessibilityRelationBDD) const {
  const PMUInt16 previousVariableCount = (PMUInt16) mPreviousVariableCount.uintValue () ;
  const PMUInt16 totalVariableCount = (PMUInt16) mTotalVariableCount.uintValue () ;
//--- Compute operand
...
//--- Construct substitution arraies
  ...
  while (p != NULL) {
    macroValidPointer (p) ;
    statesSubstitutionArray [index] = (PMUInt16) p->mTargetSlot.uintValue () ;
    transitionsSubstitutionArray [index] = (PMUInt16) p->mTargetSlot.uintValue () ;
    transitionsSubstitutionArray [importedMachineVariableCount + index] = (PMUInt16) (inVariablesCount + p->mTargetSlot.uintValue ()) ;
    p = p->nextObject () ;
    index ++ ;
  }
//--- Translate initial state BDD
...
//--- Compute BDDs for each mode
  const PMSInt32 modeCount = (PMSInt32) mModeMap.count () ;
...
//--- Add to accessibility relation transition from terminal states to initial state (if accepted)
  ...
  while (currentInclusion != NULL) {
    const PMSInt32 sourceMode = (PMSInt32) currentInclusion->mSourceMode.uintValue () ;
    const PMSInt32 targetMode = (PMSInt32) currentInclusion->mTargetMode.uintValue () ;
  //--- translate initial state BDD by inVariablesCount slots
    ...
    for (PMSInt32 targetMode=0 ; targetMode < modeCount ; targetMode++) {
      ...
      //--- Is theses transitions accepted ?
        ...
        while ((currentExclusion != NULL) && isAccepted) {
          isAccepted = (sourceMode != (PMSInt32) currentExclusion->mSourceMode.uintValue ()) || (targetMode != (PMSInt32) currentExclusion->mTargetMode.uintValue ()) ;
