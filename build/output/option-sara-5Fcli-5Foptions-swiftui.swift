...
fileprivate let displayBDDvaluesCount_PREFKEY = "sara_cli_options:displayBDDvaluesCount"
fileprivate let displayBDDvalues_PREFKEY = "sara_cli_options:displayBDDvalues"
...
struct OptionView : View {
  ...
  @AppStorage(displayBDDvaluesCount_PREFKEY) private var displayBDDvaluesCount : Bool = false
  @AppStorage(displayBDDvalues_PREFKEY) private var displayBDDvalues : Bool = false
    ...
    VStack {
      ...
      ScrollView {
        VStack (alignment: .leading) {
          Toggle ("displays values count for every BDD", isOn: self.$displayBDDvaluesCount)
          Toggle ("displays values for every BDD", isOn: self.$displayBDDvalues)
        }.padding ()
      }.background (.white)
...
func commandLineToolInvocation () -> CommandLineToolInvocation {
  ...
  if ud.bool (forKey: displayBDDvaluesCount_PREFKEY) {
    arguments.append ("--display-bdd-values-count")
  }
  if ud.bool (forKey: displayBDDvalues_PREFKEY) {
    arguments.append ("--display-bdd-values")
  }
